 ...
 * DESCRIPTION:
 * Drives all 4 digits on a 3461AS display to automatically cycle from 
 * 0000 to 9999. Splits the segment bitmasks across PORTC (segments A-F) 
 * and PORTA (segment G and DP). Explicitly injects a static decimal point 
 * on the thousands digit. Uses a precise 16-bit Timer1 interrupt setup with 
 * a 1:8 prescaler to generate 1 ms ticks. The main loop tracks these ticks to 
 * update the data value every 100 ms, while an FSM handles multiplexing.
 * ========================================================================== */
...
// Array containing segment configurations for PORTA (Segment G control)
            ...
            case STATE_SEGMENT:
                if(display_number == 1){
                    ...
                    if(number_base_1000 == 0)
                        ...
                        PORTA |= (number_PORTA[number_base_1000] | seg_P);
